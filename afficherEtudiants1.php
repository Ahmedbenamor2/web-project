...
<div class="container" >
<div class="row">
<div class="table-responsive"id="resultats"> 
  <div class="container">
    <div class="row">
 ...
 <button  type="button" id="button1" class="btn btn-primary btn-block active">Actualiser</button>
</div>
</div>
<script>
  ...
  function function1(){
    ...
    xhr.open('GET','res_affichage.php',true);
    xhr.onload=function(){
        if (this.status==200){
          console.log(this.responseText);
            ...
            for (var i in user){
                ...
                output+='<tr>'+'<td>'+user[i].cin+'</td>'+'<td>'+user[i].email+'</td>'+'<td>'+user[i].nom+'</td>'+'<td>'+user[i].prenom+'</td>'+'<td>'+user[i].adresse+'</td>'+'<td>'+user[i].Classe+'</td>'+'</tr>' ;
            }
